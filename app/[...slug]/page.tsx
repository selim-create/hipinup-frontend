...
import { CategoryPage, ArticlePage } from "@/components/magazine";
...
export default async function Page({params,searchParams}:Props){const {article,category,page}=resolve((await params).slug);if(article)return <ArticlePage article={article}/>;if(category){const p=page||Number((await searchParams).page||1);return <CategoryPage category={category} page={Number.isFinite(p)?Math.floor(p):1}/>;}notFound();}
